...
//Initialize the map index _and register file
...
/*
This part finishes the decode part _and lists aLL the R-TYPE instructions
 */
...
void addw(ULL rs1Val, ULL rs2Val, int rdInt) {
	/*
	Overfows are ignored, _and the low 
	32-bits of the result is sign-extended to 64-bits 
	_and written to the destination register.
	 */
	int src_1 = rs1Val;
	int src_2 = rs2Val;
	int sum = src_1 + src_2;
...
/*
This part finishes the decode part of I-TYPE _and lists aLL the I-TYPE instructions
 */
...
void lb(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	LL loadData = (LL)mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 1 );
...
void lh(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	LL loadData = (LL)mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 2 );
...
void lw(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	LL loadData = (LL)mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 4 );
...
void ld(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	LL loadData = (LL)mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 8 );
	reg.setIntRegVal((ULL)loadData, rdInt);
}
void lbu(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	ULL loadData = mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 1 );
...
void lhu(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	ULL loadData = mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 2);
...
void lwu(string instruction) {
	...
	LL rs1Val = (LL)reg.getIntRegVal(rs1Int);
	ULL loadData = mymem.rwmemRead( (ULL)(immediateNum + rs1Val), 4 );
	long long mask = ((long long) 1 << 63) >> 31;
...
void srliw(string instruction) {
	/*
	operate on 32-bit values _and produce signed 32-bit results.
	 */
	int shamt = (content >> 20) & 31;
	int rs1Int = (content >> 15) & 31;
	int rdInt = (content >> 7) & 31;
...
/*
This part finishes the decode part of S-TYPE _and lists aLL the S-TYPE instructions
 */
void sb(string instruction) {
	...
	LL immediateNum = ((immediateHigherPart + immediateLowerPart) << 52) >> 52;	//get the immediate number _and get sign-extended
	...
	mymem.rwmemWrite((ULL)memoryAddr, rs2Val, 1);
}
void sh(string instruction) {
	...
	mymem.rwmemWrite((ULL)memoryAddr, rs2Val, 2);
}
void sw(string instruction) {
	...
	mymem.rwmemWrite(memoryAddr, rs2Val, 4);
}
void sd(string instruction) {
	...
	mymem.rwmemWrite(memoryAddr, rs2Val, 8);
}
...
/*
This part finishes the decode part of SB-TYPE _and lists aLL the SB-TYPE instructions
 */
...
/*
This part finishes the decode part of UJ-TYPE _and lists aLL the UJ-TYPE instructions
 */
...
void systemCall(int sys_call_num)
{}
...
void getOpcode(string instruction) {
	...
	switch(atoi(typeIndex[opcode].c_str())) {
		case 1:
...
void decode(ULL startAddr) {
	Initialize(startAddr);		//Initialize some variables _and prepare for the decode part
